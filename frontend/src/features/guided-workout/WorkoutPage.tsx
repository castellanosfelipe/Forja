...
export function WorkoutPage() {
  ...
  async function start(day: PlanDay) {
    ...
    try {
      ...
      setMessage(cause instanceof Error ? cause.message : 'No pudimos iniciar la sesión.');
  ...
  async function completeWorkout() {
    ...
    try {
      await flush();
      if (useStateStore.getState().hasPendingChanges) throw new Error('La sesión sigue guardada localmente, pero aún no pudo sincronizarse. Reintenta antes de calcular la progresión.');
      await stateApi.completeWorkout(active.id);
      await refresh();
      await timer.cancel();
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : 'No pudimos completar la sesión.'); }
    finally { setStarting(false); }
  }
  ...
  if (!current || !exercise) return <main className="page"><div className="empty-state"><Dumbbell size={32} /><h1>No podemos continuar esta sesión</h1><p>Uno de sus ejercicios ya no existe en la biblioteca. Cancela la sesión para volver a elegir una rutina.</p><button className="danger-button" type="button" onClick={() => void cancelWorkout()}>Cancelar sesión dañada</button></div></main>;

  return (
    <main className="guided-workout">
      <header className="workout-topbar"><button type="button" className="text-link" onClick={() => setConfirmExit(true)}><ArrowLeft size={18} /> Salir</button><div><span>{completedSets} de {totalSets} series</span><div className="progress-track" role="progressbar" aria-label="Progreso de la sesión" aria-valuemin={0} aria-valuemax={totalSets} aria-valuenow={completedSets}><i style={{ width: `${totalSets ? completedSets / totalSets * 100 : 0}%` }} /></div></div><span className={wakeLock.active ? 'wake-status active' : 'wake-status'} role="status"><Smartphone size={15} /> {wakeLock.supported ? wakeLock.active ? 'Pantalla activa' : 'Wake Lock pausado' : 'Wake Lock no disponible'}</span></header>
      ...
      <section className="workout-stage">
        <aside className="exercise-rail" aria-label="Ejercicios de la sesión">{active.exercises.map((item, index) => { const library = state.exerciseLibrary.find((candidate) => candidate.id === item.exerciseId); const complete = item.sets.every((set) => set.completedAt); return <button type="button" aria-current={index === exerciseIndex ? 'step' : undefined} className={index === exerciseIndex ? 'active' : ''} key={`${item.exerciseId}-${index}`} onClick={() => setExerciseIndex(index)}><span>{complete ? <CheckCircle2 size={17} /> : index + 1}</span><div><strong>{library?.name ?? item.exerciseId}</strong><small>{item.sets.filter((set) => set.completedAt).length}/{item.sets.length} series</small></div></button>; })}</aside>
