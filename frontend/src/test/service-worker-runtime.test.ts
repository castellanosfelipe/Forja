...
function runtime(){
  ...
  runInNewContext(worker,{URL,caches,fetch:fetcher,self:{location:{origin:'https://qa.local'},addEventListener:(name:string,handler:any)=>handlers.set(name,handler),skipWaiting,clients:{claim:vi.fn()},registration:{showNotification:vi.fn()}}});
  ...
  return {caches,fetcher,event,cacheMaps,skipWaiting};
}
...
it('PWA-045 installing an update keeps the active version available to open workout tabs', async () => {
  const r = runtime();
  const previous = await r.caches.open('forja-shell-previous-build');
  await previous.put('/assets/previous-workout.js', new Response('previous lazy route'));
  await r.event('install');
  expect(r.skipWaiting).not.toHaveBeenCalled();
  expect(await (await previous.match('/assets/previous-workout.js'))?.text()).toBe('previous lazy route');
  await r.event('activate');
  expect(r.cacheMaps.has('forja-shell-previous-build')).toBe(false);
});
